...
def build_uv_texture_list(i_vert,i_face,me,obj):
    ...
    for uvt in me.uv_layers:
        ...
        if me_img==None:
            ...
            if not(os.path.isfile(texture)) or me_img.file_format!='TARGA':
...
#            break
#        ligne = vert.replace("\n","").split(" ")
#        for vertices in ligne :
#            l_int.append(int(float(vertices)))
#        loops_vert_idx.extend(l_int)
#        
...
#                    me.vertex_colors[ntc-1].data[nf].color3=[float(l[0]),float(l[1]),float(l[2])]
                if (nv%3)==0:
                    me.vertex_colors[ntc-1].data[nf].color[1]=[float(l[0]),float(l[1]),float(l[2])][0]
                if (nv%3)==1:
                    me.vertex_colors[ntc-1].data[nf].color[2]=[float(l[0]),float(l[1]),float(l[2])][0]
                if (nv%3)==2:
                    me.vertex_colors[ntc-1].data[nf].color[3]=[float(l[0]),float(l[1]),float(l[2])][0]
            else:
                ntt=ntt+1
                #print (t)
        ...
        if (nv%3)==0:
            ...
            me.loops.foreach_set("vertex_index", [(nv-3),(nv-2),(nv-1)])
